...
from PyQt6.QtWidgets import (QWidget, QApplication, QTableWidget, QTableWidgetItem, QLineEdit, QTextEdit, QAbstractItemView, 
                             QVBoxLayout, QPushButton, QToolBar, QMessageBox, QLabel, QMainWindow, QAbstractItemView)
...
class Viewer(QWidget):
    def __init__(self):
        ...
        btn_close = QPushButton(text = "Schließen")
        btn_close.setFixedWidth(100)
        btn_close.setFixedHeight(28)
        btn_close.clicked.connect(self.close_me)
...
class Window(QMainWindow):
    def __init__(self):
        ...
        tbf = QToolBar("Tools")
        tbf.setContextMenuPolicy(Qt.ContextMenuPolicy.PreventContextMenu)
        tbf.setMovable(False)
        self.addToolBar(Qt.ToolBarArea.TopToolBarArea, tbf)
        empty = QWidget()
        ...
        act9 = QPushButton("9:00", self, clicked = lambda: self.doTime("9:"))
        act9.setFixedWidth(70)
        tbf.addWidget(act9)

        act12 = QPushButton("12:00", self, clicked = lambda: self.doTime("12:"))
        act12.setFixedWidth(70)
        tbf.addWidget(act12)
        ...
        tb2.setContextMenuPolicy(Qt.ContextMenuPolicy.PreventContextMenu)
    ...
    def doTime(self, ft):
        ...
        for row in range(self.table.rowCount()):
            for column in range(self.table.columnCount()):
                if not self.table.item(row, column) == None:
                    if self.table.item(row, column).text().startswith(ft):
                        header = self.table.horizontalHeaderItem(column).text()
                        m = self.table.item(row, column).text().replace("20:15 ", "", 1)
                        msg = "<b style='color:#729fcf'>head</b><br>msg<br>"
        ...
        for row in range(self.table.rowCount()):
            ...
            if not rowtext == "":
                ...
                if mt > k and mt < m:
                    ...
                    self.table.scrollToItem(self.table.selectedItems()[0], QAbstractItemView.ScrollHint.PositionAtCenter)
                else:
                    self.table.item(row, column).setSelected(False)
        #for row in range(self.table.rowCount()):
        if self.table.selectedItems() == []:
            ...
            for row in range(self.table.rowCount()):
                ...
                if not rowtext == "":
                    mt = int(rowtext)
                    #if mt > k and mt < m:
                    if rowtext.startswith(str(h)):
                        self.table.item(row, column).setSelected(True)
                        self.table.scrollToItem(self.table.selectedItems()[0], 1)
                    elif rowtext.startswith(str(h-1)):
                        self.table.item(row, column).setSelected(True)
                        self.table.scrollToItem(self.table.selectedItems()[0], 1)
                    else:
                        self.table.item(row, column).setSelected(False)                
    ...
    def doNow(self):
        ...
        self.v = Viewer()
        self.v.title.setText("<h1 style='color:yellow'>jetzt im TV</h1>(Schließen mit Escape)")
    ...
    def doNext(self):
        ...
        self.v = Viewer()
        self.v.viewer.setText(text)
        self.v.title.setText("<h1 style='color:yellow'>demnächst im TV</h1>(Schließen mit Escape)")
        self.v.setGeometry(100, 50, 700, 600)
        self.v.show()
        self.v.viewer.setFocus()
...
border: 1px solid #1a2334;
selection-background-color: #f6d32d;
color: #d3d7cf;
outline: 0;
} 
QHeaderView::section
